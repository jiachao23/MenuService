...
public class XPrinter58 implements Printable
{
	private PrinterParas paras;
	...
	public XPrinter58(PrinterParas _paras)
	{
		this.paras = _paras;
	}
			...
			throws PrinterException
	{
		...
		case 0:
			...
			g2.drawString(paras.getName(),sX+(130-strWidth)/2,sY);
			//L2:打印单据名称
			strWidth = g2.getFontMetrics().stringWidth(paras.getPaperName());
			g2.drawString(paras.getPaperName(),sX+(130-strWidth)/2,sY+lineHeight*(++num));
			//L3:打印空行
			g2.drawLine(sX,sY+lineHeight*(++num),400,sY+lineHeight*(num));
			//L4:打印其它附加信息
			g2.drawString("台号:"+paras.getTableName()+"    人数："+paras.getGuestNumbers(),sX,sY+lineHeight*(++num));
			g2.drawString("单号:"+paras.getOrderNumber(),sX,sY+lineHeight*(++num));
			g2.drawString("流水号："+paras.getSerialnumber(),sX,sY+lineHeight*(++num));
			g2.drawLine(sX,sY+lineHeight*(++num),400,sY+lineHeight*(num));
			g2.drawString("下单时间："+paras.getOrderTime(),sX,sY+lineHeight*(++num));
			g2.drawLine(sX,sY+lineHeight*(++num),400,sY+lineHeight*(num));
			g2.drawString("菜名       数量       口味",sX,sY+lineHeight*(++num));
			/////////////////////////////////////////////////////////////////////
			...
			for(int i=0;i<paras.getMenuList().length;i++)
			{
				...
				g2.drawString(list[i][1],sX+70,sY+lineHeight*(num));
				g2.drawString(list[i][2],sX+110,sY+lineHeight*(num));
			}
			g2.drawLine(sX,sY+lineHeight*(++num),400,sY+lineHeight*(num));
			g2.drawString("服务员:"+paras.getWaiter()+"      打印员:"+paras.getPrinter(),sX,sY+lineHeight*(++num));
			return PAGE_EXISTS;
		default:
			return NO_SUCH_PAGE;
		}
