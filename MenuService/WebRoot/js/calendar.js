...
Calendar = function (mondayFirst, dateStr, onSelected, onClose) {
	...
	this.isPopup = true;
	this.weekNumbers = false;//true;//涓嶈褰撳墠鍛ㄦ棩鏈�
...
Calendar.dayMouseOver = function(ev) {
	...
	if (el.ttip) {
		if (el.ttip.substr(0, 1) == "_") {
			...
			el.ttip = "鏄熸湡"+date.print("D")+"  " +date.print("y")+ "骞�"+date.print("M")+date.print("d") +"鏃�";
		}
...
Calendar.dayMouseOut = function(ev) {
	with (Calendar) {
		...
		//娉ㄩ噴绉诲紑榧犳爣鏃剁殑鎻愮ず;鍥犱负鍙充笂瑙掕鏀逛负鍥剧墖;
		return stopEvent(ev);
	}
};
...
Calendar.cellClick = function(el) {
	...
	if (typeof el.navtype == "undefined") {
		...
		cal.callCloseHandler();//鍗曞嚮涔熷紩鐢ㄥ叧闂�added by jarry
	} else {
		if (el.navtype == 200) {
			Calendar.removeClass(el, "hilite");
...
Calendar.prototype.create = function (_par) {
	...
	var hh = function (text, cs, navtype) {
		...
		if (text.substr(0, 1) != "&") {
			...
			cell.innerHTML = text;//濡傛灉鏄浘鐗囩洿鎺ユ坊鍔�
			else 
...
//	var title_length = 7;//鍘熸槸6锛屽幓鎺変簡鍛ㄥ垏鎹㈡晠澧炲姞涓�釜;
...
//	(this.weekNumbers) && ++title_length;
	...
	this._nav_py = hh("<img id=objyprev src=\"../../images/yprev.gif\">", 1, -2);
...
//	this._nav_py = hh("&#x00ab;", 1, -2);
	...
	//	this._nav_ny = hh("&#x00bb;", 1, 2);
	this._nav_ny = hh("<img id=objynext src=\"../../images/ynext.gif\">", 1, 2);
	this._nav_ny.width = 25;
	this._nav_ny.ttip = Calendar._TT["NEXT_YEAR"];

//	this._nav_pm = hh("&#x2039;", 1, -1);
	this._nav_pm = hh("<img id=objmprev src=\"../../images/mprev.gif\">", 1, -1);
...
//	this._nav_nm = hh("&#x203a;", 1, 1);
	this._nav_nm = hh("<img id=objmnext src=\"../../images/mnext.gif\">", 1, 1);
...
//	this._nav_now.ttip = Calendar._TT["GO_TODAY"];
	...
	if (this.isPopup) {
		...
		//this.title.style.cursor = "move";//娉ㄩ噴绉诲姩鐘舵�
		//hh("&#x00d7;", 1, 200).ttip = Calendar._TT["CLOSE"];//鍘绘帀鍏抽棴绗︼紝浠ュ浘鐗囨浛浠�
		var td_close = hh("<img id=objclose src=\"../../images/close.gif\">", 1, 200);
		
		td_close.ttip = Calendar._TT["CLOSE"];
		td_close.align = "right";
		//hh("#<div><img src=\"/images/calendar/close.gif\" onmousedown=\"return;\"></div>", 1, 200).ttip = Calendar._TT["CLOSE"];
	}	
	...
	if (this.isPopup) {
		...
		//cell.style.cursor = "move";//娉ㄩ噴鎷栧姩鐘舵�
	}
	this.tooltips = cell;
...
//	this.title.firstChild.data = year + "骞� + Calendar._MN[month];
...
Calendar.prototype._dragStart = function (ev) {
	...
	this.dragging = false;// true;鍘绘帀鎷栧姩
	var posX;
	var posY;
	if (Calendar.is_ie) {
		posY = window.event.clientY + document.body.scrollTop;
	...
	with (Calendar) {
		//addEvent(document, "mousemove", calDragIt);//娉ㄩ噴鎷栧姩
...
// tooltips
Calendar._TT = {};
Calendar._TT["TOGGLE"] = "鍒囨崲鍛ㄦ樉绀烘柟寮�";
Calendar._TT["PREV_YEAR"] = "涓婁竴骞�(鎸変綇榧犳爣鍑轰笅鎷夎彍鍗�";
Calendar._TT["PREV_MONTH"] = "涓婁竴鏈�(鎸変綇榧犳爣鍑轰笅鎷夎彍鍗�";
Calendar._TT["GO_TODAY"] = "鍒颁粖鏃�";
Calendar._TT["NEXT_MONTH"] = "涓嬩竴鏈�(鎸変綇榧犳爣鍑轰笅鎷夎彍鍗�";
Calendar._TT["NEXT_YEAR"] = "涓嬩竴骞�(鎸変綇榧犳爣鍑轰笅鎷夎彍鍗�";
Calendar._TT["SEL_DATE"] = "閫夋嫨鏃ユ湡";
Calendar._TT["DRAG_TO_MOVE"] = "鎷栧姩";
Calendar._TT["DRAG_TO_MOVE"] = "";
Calendar._TT["PART_TODAY"] = " (浠婃棩)";
Calendar._TT["MON_FIRST"] = "棣栧厛鏄剧ず鏄熸湡涓�";
Calendar._TT["SUN_FIRST"] = "棣栧厛鏄剧ず鏄熸湡鏃�";
