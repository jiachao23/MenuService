...
<html>
	...
	<body>
					...
					<td align='center'>
						<table border='0' cellpadding='0' cellspacing='0'>
							<tr>
								...
								<td width='160'>
									<input id="menuType_Name" name='menuTypeForm.menuType_Name' type='text'
										class="txt" style='width: 150px'
										value="<s:property value='#request.menuTypeForm.menuType_Name' />" />
									<!-- 隐藏属性  说明 是 要查询数据 -->
									<input id="menuTypeForm.query" name='menuTypeForm.query' type='hidden' value="true" />
								</td>
								<td>
									<button class="btn1"
		...
		<div class="tform" style="width:80%">
			<button type='button' class="btn1" onclick="location.href='../menu/menuType/add_menuType.jsp'">
				新增菜谱类型
			</button>
	...
	<script type="text/javascript">
		...
		var recordtotal ="<s:property value="#request.menuTypeCount"/>"; 
	 	//查询到 数据条数时 并返回了  是否是 有查询条件  action_recordtotal.action 返回的
	   	var query ="<s:property value="#request.menuTypeForm.query"/>";
		...
		function InitTable(pageIndex) { 
			var menuType_Name=$("#menuType_Name").val(); 
			$("#recordlist").mask("数据加载中...");
			$.ajax({  
				type: "POST",  
				url: "../menuType/getMenuTypeList",  
				cache:false,
				data: {"menuTypeForm.pageIndex":pageIndex,"menuTypeForm.pageSize":items_per_page,"menuTypeForm.menuType_Name":menuType_Name,"menuTypeForm.query":query},  
				dataType: 'json',  
				...
				success: function(data){
					...
					$.each(data,function(i,value){
						var recordlistcontent="<tr><td align='left' style='width:10%'><input type='checkbox' value='"+value.id+"' name='wait_checked_"+value.id+"' /></td>"+
				   			...
				   			"<td align='left' style='width:10%'>"+value.menuType_No+"</td>"+  
					       	"<td align='left' style='width:10%'>"+value.menuType_Name+"</td>";
					       	
					       	recordlistcontent+="<td align='left' style='width:10%' class='ta_r'><a href='updateMenuTypePage?menuType.id="+value.id+"'>编辑</a></td></tr>";
		...
		if(idlist.length!=0)
		{
			if(confirm("确定要删除选中数据吗？"))
			{
				$.ajax({  
					type: "POST",  
					url: "deleteMenuType",  
					data: "menuTypeForm.idList="+idlist,  
	 				dataType: 'text',  
					contentType: "application/x-www-form-urlencoded",  
					success: function(data){
						art.dialog({
